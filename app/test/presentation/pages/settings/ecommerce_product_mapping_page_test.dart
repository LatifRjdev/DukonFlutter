...
void main() {
  ...
  void stubHappyPath() {
    ...
    when(
      () => dioClient.get<dynamic>('/stores/store-1/ecommerce/mappings'),
    ).thenAnswer(
      (_) async => resp<dynamic>([
  ...
  group('EcommerceProductMappingPage', () {
    ...
    testWidgets('tapping save sends a PUT with the entered external id', (
      ...
      when(
        ...
        (_) async =>
            resp<dynamic>({'productId': 'p2', 'externalProductId': 'ext-2'}),
      );
      ...
      await tester.enterText(
        find.byKey(const ValueKey('mapping-external-id-p2')),
        'ext-2',
      );
    ...
    testWidgets('typing in the search field filters the product list by name', (
      ...
      await tester.enterText(
        find.byKey(const Key('mapping-search-field')),
        'товар 1',
      );
      await tester.pump();
