...
| 25 | `/suppliers` (Ещё → Поставщики) | Supplier list | ✗ | — | **FD-026 P0 CONFIRMED**: tapping the "Поставщики" ListTile in Ещё menu triggers `GET /stores/:id/customers` (verified via Flutter logcat `I flutter : uri: http://10.0.2.2:4455/api/stores/.../customers?page=1&limit=20`) and renders the Customers screen with title "Клиенты" and customer filter chips. Then CRASHES with the same `List → Map` cast error. Source code (`more_page.dart:98`) says `context.push(RouteNames.supplierList)` and router (`app_router.dart:323`) maps `/suppliers → SupplierListPage()`, so the source path looks correct — yet the running APK navigates to CustomerListPage. Possible causes: (a) stale APK install vs source; (b) hit-test overlap between Клиенты [0,1452-1534] and Поставщики [0,1534-1681] tiles causing tap to fall through to customers; (c) route mismatch in go_router not yet diagnosed. **Either way, end-user impact is the same: tapping "Поставщики" never reaches SupplierListPage, and also crashes.** Needs root-cause investigation in fix PR. |
...
| FD-026 | **P0** | Suppliers menu item navigates to Customers (wrong route or hit-test overlap) then crashes with same List→Map cast | /suppliers |
