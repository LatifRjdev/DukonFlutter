...
11. **FD-P0-005 (FD-026)** — **Поставщики menu item is unreachable + crash**. **Confirmed** via Flutter logcat: tapping "Поставщики" in Ещё menu triggers `GET /stores/:id/customers` and renders the Customers screen (title "Клиенты", customer filter chips) *then* crashes with the same `List → Map` cast error. Source code in `more_page.dart:98` pushes `RouteNames.supplierList` and `app_router.dart:323` maps it to `SupplierListPage`, but the running APK behaves differently — either stale build or hit-test overlap between Клиенты [y=1452-1534] and Поставщики [y=1534-1681] tiles. **End-user impact:** Suppliers feature is completely inaccessible from the UI.
